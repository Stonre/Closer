...
class LoginViewController: UIViewController {
    ...
    private func setupLoginRegisterSegmentedControl() {
        ...
        loginRegisterSegmentedControl.widthAnchor.constraint(equalTo: view.widthAnchor, constant: -16).isActive = true
        ...
        FIRAuth.auth()?.createUser(withEmail: email, password: password) { (user, error) in
            ...
            FIRAuth.auth()?.signIn(withEmail: email, password: password, completion: { (user, error) in
                ...
                if let currUserID = FIRAuth.auth()?.currentUser?.uid {
                    ...
                    let ref = FIRDatabase.database().reference(fromURL: "https://closer-17ee2.firebaseio.com/")
